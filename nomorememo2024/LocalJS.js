...
class tabElement {
    ...
    tabMainPage() {
        ...
        } else if (this.db.type == 2) {
            const calcul = new tabElement_link();
            calcul.setValue(this.db);
            tabDiv.appendChild(calcul.setElementAll());
        }
...
class tabElement_calcul {
    ...
    function_checkEvent(event) {
        ...
        if (target.includes("textDiv") || target.includes("emptyDiv") || target.includes("mark")) {
            ...
            observer.target = "tab_calcul_text_U";
            observer.value = {
                index: target.className.split("_")[1].replace(baseic_regex, ""),
                key: "checked", value: event.target.checked
            };
            subj.subscribe(observer);
            subj.notifyAll();
            ...
            if (value != null && event.target.checked) {
                sumDiv.innerText = Number(value2) + Number(value);
...
class tabElement_link {
    ...
    constructor() {
        ...
        this.option = document.createElement("option");
        this.textarea = document.createElement("textarea");
        this.mark = document.createElement("a");
    }
    setValue(db) {
        this.db.textArr = db.tabInfo.text;
        this.db.colorArr = db.tabInfo.color;
        ...
        this.mark.style.fontFamily = this.db.fontFamily;
        this.mark.style.fontSize = `${this.db.fontSize * 2}pt`;
        this.mark.style.fontWeight = this.db.fontWeight;
        this.mark.style.fontStyle = this.db.fontStyle;
        this.mark.style.color = this.db.fontColor;
    ...
    function_newTuple(event) {
        ...
        observer.check = true;
        observer.name = "tap_link_text update event";
        observer.target = "tab_link_text_C";
        observer.value = { tabIndex: tabIndex, text: text, colorIndex: colorIndex };
        subj.subscribe(observer);
        subj.notifyAll();
...
class Model {
    ...
    DBname = {
        ...
        tab_link_tag: "link_tag",
        tab_link_text: "link_text",
    }
    constructor() {
        ...
        this.tab_link_tagArr = JSON.parse(localStorage.getItem(this.DBname.tab_link_tag));
        this.tab_link_textArr = JSON.parse(localStorage.getItem(this.DBname.tab_link_text));
        ...
        //2
        if (this.tab_link_tagArr == null) {
            this.tab_link_tagArr = [];
            this.tab_link_tag_save();
        } else {
            let newArr = []
            for (let i = 0; i < this.tab_link_tagArr.length; i++) {
                if (this.tab_link_tagArr != null) {
                    newArr.push(this.tab_link_tagArr[i]);
                }
            }
            this.tab_link_tagArr = newArr;
            this.tab_link_tag_save();
        }
        if (this.tab_link_textArr == null) {
            this.tab_link_textArr = [];
            this.tab_link_text_save();
        } else {
            let newArr = []
            for (let i = 0; i < this.tab_link_textArr.length; i++) {
                if (this.tab_link_textArr != null) {
                    newArr.push(this.tab_link_textArr[i]);
                }
            }
            this.tab_link_textArr = newArr;
            this.tab_link_text_save();
        }
    }
    //html_remote
    ...
    tab_C(winIndex, type) {
        ...
        if (type == 0) {
            ...
            this.tab_link_tag_C(newTab.index);
        }
    }
    ...
    //2
    //tab_link_tag
    tab_link_tag_C(tabIndex) {
        const newColor = new Tab_Link_tag();
        newColor.fk_tabIndex = tabIndex;
        this.tab_link_tagArr.push(newColor);
        this.tab_link_tag_save();
        console.log("mod",this.tab_link_tagArr);
    }
    tab_link_tag_U(tabIndex, key, value) {
        for (let i = 0; i < this.tab_link_tagArr.length; i++) {
            if (this.tab_link_tagArr[i].fk_tabIndex == tabIndex) {
                this.tab_link_tagArr[i][key] = value;
            }
        }

        this.tab_link_tag_save();
    }
    tab_link_tag_D(tabIndex) {
        let newArr = [];
        for (let i = 0; i < this.tab_link_tagArr.length; i++) {
            if (this.tab_link_tagArr[i].fk_tabIndex != tabIndex) {
                newArr.push(this.tab_link_tagArr[i]);
    ...
    //tab_link_text
    tab_link_text_C(tabIndex, text, colorIndex) {
        const memoText = new Tab_Link_text();
        ...
        this.tab_link_textArr.push(memoText);
        this.tab_link_text_save();
        this.value = memoText;
    }
    tab_link_text_U(tupleIndex, key, value) {
        this.tab_link_textArr[tupleIndex][key] = value;
        this.tab_link_text_save();
    }
    tab_link_text_U_indexChange(tupleIndex1, tupleIndex2) {
        ...
        this.tab_link_textArr[tupleIndex2] = ex;
        this.tab_link_textArr[tupleIndex2].index = tupleIndex2;
        this.tab_link_text_save();
    }
    tab_link_text_D(textIndex) {
        //trash tab_text---
        const timeIndex = new Date();
        const newTrashTuple = {
            index: timeIndex,
            text: this.tab_link_textArr[textIndex].text
        }
        this.htmlInfo.trash.tab_text.push(newTrashTuple);
        if (this.htmlInfo.trash.tab_text.length > 40) {
            this.htmlInfo.trash.tab_text.shift();
        ...
        this.tab_link_textArr[textIndex] = null;
        this.tab_link_text_save();
    }
    ...
    //2
    tab_link_tag_save() { localStorage.setItem(this.DBname.tab_link_tag, JSON.stringify(this.tab_link_tagArr)); }
    tab_link_tag_nullClear() {
        let newArr = [];
        for (let i = 0; i < this.tab_link_tagArr.length; i++) {
            if (this.tab_link_tagArr[i] != null) {
                newArr.push(this.tab_link_tagArr[i]);
            }
        }
        this.tab_link_tagArr = newArr;
    ...
    tab_link_text_save() { localStorage.setItem(this.DBname.tab_link_text, JSON.stringify(this.tab_link_textArr)); }
    tab_link_text_nullClear() {
        let newArr = [];
        for (let i = 0; i < this.tab_link_textArr.length; i++) {
            if (this.tab_link_textArr[i] != null) {
                newArr.push(this.tab_link_textArr[i]);
            }
        }
        this.tab_link_textArr = newArr;
    ...
    constructor(type, info, tabType, tabInfo) {
        if (type == "html") {
            ...
            if (tabType == "memo") {
                ...
                const element = new tabElement_link();
                const exInfo = { text: [], color: info.color };
                info.set.tabInfo = exInfo;
...
class Controller {
    ...
    firstPageOpen() {
        ...
        while (true) {
            ...
            if (this.model.tabInfoArr[tab_i].type == 0) {
                ...
                this.model.tabInfoArr[tab_i].tabInfo = tabInfo;
            } else if (this.model.tabInfoArr[tab_i].type == 2) {
                let tabText = [];
                let tabTag = [];
                for (let j = 0; j < this.model.tab_link_textArr.length; j++) {
                    if (this.model.tab_link_textArr[j] != null && this.model.tab_link_textArr[j].fk_tabIndex == tab_i) {
                        this.model.tab_link_textArr[j].index = j;
                        tabText.push(this.model.tab_link_textArr[j]);
                    }
                }
                for (let j = 0; j < this.model.tab_link_tagArr.length; j++) {
                    if (this.model.tab_link_tagArr[j] != null && this.model.tab_link_tagArr[j].fk_tabIndex == tab_i) {
                        tabTag = this.model.tab_link_tagArr[j];
                    }
                }
                const tabInfo = { text: tabText, color: tabTag, winSelect: winSelectInfo };
                this.model.tabInfoArr[tab_i].tabInfo = tabInfo;
            }
    ...
    getValue(target, value) {
        if (target == "window_C") {
        ...
        } else if (target == "tab_link_text_C") {
            const tabIndex = value.tabIndex;
            const text = value.text;
            const colorIndex = value.colorIndex;
            ...
            for (let j = 0; j < this.model.tab_link_tagArr.length; j++) {
                if (this.model.tab_link_tagArr[j] != null && this.model.tab_link_tagArr[j].fk_tabIndex == tabIndex) {
                    info.color = this.model.tab_link_tagArr[j];
                }
            }
            this.tupleAppend(tabIndex, info, "link");

        } else if (target == "tab_link_text_U") {
            const index = value.index;
            const key = value.key;
            let value2 = value.value;
            if (value2 == "true" || value2 == "false") { value2 = value2 == "true" ? true : false; }
            ...
            const key = value.key;
            const colorValue = value.value;
            this.model.tab_link_tag_U(tabIndex, key, colorValue);
        }
    }
